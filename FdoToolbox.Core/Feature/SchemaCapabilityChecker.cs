...
namespace FdoToolbox.Core.Feature
{
    public class SchemaCapabilityChecker
    {
        ...
        public SchemaCapabilityChecker(ISchemaCapabilities schemaCaps)
        {
            _schemaCaps = schemaCaps;
        }
        ...
        public bool CanApplyClass(ClassDefinition classDef, out IncompatibleClass cls)
        {
            ...
            if (Array.IndexOf<ClassType>(_schemaCaps.ClassTypes, ctype) < 0)
            {
            ...
            if (!_schemaCaps.SupportsCompositeId && classDef.IdentityProperties.Count > 1)
            {
            ...
            if (!_schemaCaps.SupportsInheritance && classDef.BaseClass != null)
            {
            ...
            foreach (PropertyDefinition propDef in classDef.Properties)
            {
                ...
                if (!_schemaCaps.SupportsAutoIdGeneration)
                {
                ...
                else
                {
                    if (dataDef != null && dataDef.IsAutoGenerated)
                    {
                        if (Array.IndexOf<DataType>(_schemaCaps.SupportedAutoGeneratedTypes, dataDef.DataType) < 0)
                        {
                ...
                {
                    if (Array.IndexOf<DataType>(_schemaCaps.SupportedIdentityPropertyTypes, dataDef.DataType) < 0)
                    {
                ...
                if (!_schemaCaps.SupportsAssociationProperties)
                {
                ...
                if (!_schemaCaps.SupportsDefaultValue)
                {
                ...
                if (!_schemaCaps.SupportsExclusiveValueRangeConstraints)
                {
                ...
                if (!_schemaCaps.SupportsInclusiveValueRangeConstraints)
                {
                    if (dataDef != null && dataDef.ValueConstraint != null && dataDef.ValueConstraint.ConstraintType == PropertyValueConstraintType.PropertyValueConstraintType_Range)
                    {
                        PropertyValueConstraintRange range = dataDef.ValueConstraint as PropertyValueConstraintRange;
                ...
                if (!_schemaCaps.SupportsNullValueConstraints)
                {
                ...
                if (!_schemaCaps.SupportsObjectProperties)
                {
                    if (objDef != null)
                    {
                ...
                if (!_schemaCaps.SupportsUniqueValueConstraints)
                {
                    //
                }
                if (!_schemaCaps.SupportsValueConstraintsList)
                {
                ...
                if (dataDef != null)
                {
                    if (Array.IndexOf<DataType>(_schemaCaps.DataTypes, dataDef.DataType) < 0)
                    {
                ...
                if (dp != null)
                {
                    switch (dp.DataType)
                    {
                        case DataType.DataType_BLOB:
                            {
                                int length = (int)_schemaCaps.get_MaximumDataValueLength(DataType.DataType_BLOB);
                        ...
                        case DataType.DataType_CLOB:
                            {
                                int length = (int)_schemaCaps.get_MaximumDataValueLength(DataType.DataType_CLOB);
                                if (dp.Length > length && length > 0)
                                    dp.Length = length;
                            }
                            break;
                        case DataType.DataType_String:
                            {
                                int length = (int)_schemaCaps.get_MaximumDataValueLength(DataType.DataType_String);
                                if (dp.Length > length && length > 0)
                                    dp.Length = length;
                            }
                            break;
                        case DataType.DataType_Decimal:
                            {
                                if (dp.Precision > _schemaCaps.MaximumDecimalPrecision)
                                    dp.Precision = _schemaCaps.MaximumDecimalPrecision;

                                if (dp.Scale > _schemaCaps.MaximumDecimalScale)
                                    dp.Scale = _schemaCaps.MaximumDecimalScale;
                            }
                            break;
                    }
                }
                    ...
                    case IncompatibleClassReason.UnsupportedCompositeKeys:
                        {
                            ...
                            DataType[] idTypes = new DataType[_schemaCaps.SupportedAutoGeneratedTypes.Length];
                            Array.Copy(_schemaCaps.SupportedAutoGeneratedTypes, idTypes, idTypes.Length);
            ...
            foreach (IncompatiblePropertyReason reason in reasons)
            {
                switch (reason)
                {
                    ...
                    case IncompatiblePropertyReason.UnsupportedDataType:
                        {
                            ...
                            DataType dt = GetPromotedDataType(dp.DataType, _schemaCaps.DataTypes);
                            dp.DataType = dt;
                            if (dp.DataType == DataType.DataType_BLOB ||
                                dp.DataType == DataType.DataType_CLOB ||
                                dp.DataType == DataType.DataType_String)
                            {
                                dp.Length = (int)_schemaCaps.get_MaximumDataValueLength(dp.DataType);
                            }
                    ...
                    case IncompatiblePropertyReason.UnsupportedAutoGeneratedType:
                        {
                            ...
                            if (dp.DataType == DataType.DataType_BLOB ||
                                ...
                                dp.DataType == DataType.DataType_String)
                            {
                                long length = _schemaCaps.get_MaximumDataValueLength(dp.DataType);
                                if (length <= 0)
                                    length = 255;
                    ...
                    case IncompatiblePropertyReason.UnsupportedIdentityPropertyType:
                        {
                            //Try to promote data type
                            DataType dt = GetPromotedDataType(dp.DataType, _schemaCaps.SupportedIdentityPropertyTypes);
                            dp.DataType = dt;
                            if (dp.DataType == DataType.DataType_BLOB ||
                                dp.DataType == DataType.DataType_CLOB ||
                                dp.DataType == DataType.DataType_String)
